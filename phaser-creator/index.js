      ...
      _prompt = _asyncToGenerator(function* (questions = [], {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done; ) {
            ...
            try {
              answer = prompt._injected ? getInjectedAnswer(prompt._injected, question.initial) : yield prompts9[type](question);
  ...
  "node_modules/prompts/lib/index.js"(exports2, module2) {
    ...
    async function prompt(questions = [], { onSubmit = noop2, onCancel = noop2 } = {}) {
      ...
      for (question of questions) {
        ...
        try {
          answer = prompt._injected ? getInjectedAnswer(prompt._injected, question.initial) : await prompts9[type](question);
    ...
    module2.exports = Object.assign(prompt, { prompt, prompts: prompts9, inject, override });
  }
});
  ...
  "node_modules/adm-zip/util/utils.js"(exports2, module2) {
    ...
    Utils.prototype.writeFileTo = function(path5, content, overwrite, attr) {
      ...
      if (self2.fs.existsSync(path5)) {
        if (!overwrite)
          return false;
        var stat2 = self2.fs.statSync(path5);
    ...
    Utils.prototype.writeFileToAsync = function(path5, content, overwrite, attr, callback) {
      ...
      self2.fs.exists(path5, function(exist) {
        ...
        self2.fs.stat(path5, function(err, stat2) {
    ...
    Utils.prototype.findFiles = function(path5) {
      ...
      function findSync(dir, pattern, recursive) {
        ...
        self2.fs.readdirSync(dir).forEach(function(file) {
          ...
          if (self2.fs.statSync(path6).isDirectory() && recursive)
            files = files.concat(findSync(path6, pattern, recursive));
    ...
    Utils.sanitize = function(prefix, name) {
      ...
      for (var i2 = 0, l = parts.length; i2 < l; i2++) {
        var path5 = pth.normalize(pth.join(prefix, parts.slice(i2, l).join(pth.sep)));
...
var githubURL = "https://phaser.io/r/create-game-template/template-{template-name}";
var projectConfig = {
  ...
  if (projectConfig.skeleton === "clean") {
    if (projectConfig.skeleton === "clean") {
      ...
      clean.delete.forEach((item) => {
        ...
        const path5 = `./${projectConfig.folderName}/${itemFormatted}`;
        import_fs_extra2.default.removeSync(path5);
      });
...
// src/utils/getPackageManagerName.ts
var getPackageManagerName = () => {
...
var centerWelcomeMessage = (txt, totalWidth) => {
...
var questionsText = {
  ...
  third: {
    message: source_default.yellow("Select Third Party Template:")
  },
  demo: {
    message: source_default.yellow("Select Demo Game:")
  },
  helpMessage: `
${source_default.bold("Usage:")} ${source_default.cyan(`${getPackageManagerName()} create @phaserjs/game@latest [folder-name]`)}

${source_default.bold("Options:")}
    ${source_default.yellow("--help")}            Show this message and exit
    ${source_default.yellow("--version")}         Print version info and exit

${source_default.bold("Examples:")}
    # Pass a custom folder
    > ${source_default.yellow(`${getPackageManagerName()} create @phaserjs/game@latest folder-name`)}
    ...
    > ${source_default.yellow(`${getPackageManagerName()} create @phaserjs/game@latest .`)}

For more information about templates please read the ${source_default.underline("README.md")} file inside of each template.
`,
  folderNameAction: {
  ...
  typeTemplate: {
    message: source_default.yellow("Select Option:"),
    choices: {
  ...
  cleanOrSkeleton: {
    message: source_default.yellow("Minimal or Complete Project:"),
    choices: {
...
|${centerWelcomeMessage("Let\xB4s start the installation", 54)}|
...
// src/utils/isCurrentDirectory.ts
var import_path2 = __toESM(require("path"));
var isCurrentDirectory = (inputPath) => {
  const normalizedPath = import_path2.default.normalize(inputPath);
  const currentDirectory = import_path2.default.resolve(".");
  const inputAbsolutePath = import_path2.default.resolve(normalizedPath);
...
// src/questions/getUserDesition.ts
var import_prompts5 = __toESM(require_prompts3());
var getUserDesition = async () => {
  console.log("");
...
var getTemplateLangSufix = async () => {
  ...
  if (!templateList.includes(`${projectConfig.templateName}-ts`)) {
    ...
    const lang = await (0, import_prompts6.default)([
      {
        type: "select",
        name: "sufixLang",
...
var downLoadProject = async () => {
  ...
  if (projectConfig.folderAction === "delete") {
    ...
    try {
      const deletePath = import_path4.default.join(`./${projectConfig.folderName}/`);
  ...
  zip.getEntries().forEach((entry) => {
    ...
    const destPath = import_path4.default.join(`./${projectConfig.folderName}`, `${fileName.substring(fileName.indexOf("/") + 1)}`);
    if (entry.isDirectory) {
...
var getDemo = async () => {
  ...
  const demo = await (0, import_prompts7.default)({
    type: "select",
    name: "value",
...
// src/questions/getMiddleware.ts
var import_prompts8 = __toESM(require_prompts3());
var getMiddleware = async () => {
  const middeware_group = templatesList.middleware;
  const third = await (0, import_prompts8.default)({
    type: "select",
    name: "value",
...
var downLoadProjectURL = async (url) => {
  ...
  if (projectConfig.folderAction === "delete") {
    spinner.text = questionsText.downloadTemplate.spinner.deleting;
    try {
      const deletePath = import_path5.default.join(`./${projectConfig.folderName}/`);
      import_fs_extra5.default.removeSync(deletePath);
      spinner.succeed(questionsText.downloadTemplate.spinner.deletingSuccess);
    } catch (err) {
      console.error(err);
      spinner.fail(questionsText.downloadTemplate.spinner.deletingFail);
  ...
  zip.getEntries().forEach((entry) => {
    const fileName = entry.entryName;
    const destPath = import_path5.default.join(`./${projectConfig.folderName}`, `${fileName.substring(fileName.indexOf("/") + 1)}`);
    if (entry.isDirectory) {
      import_fs_extra5.default.ensureDirSync(destPath);
    } else {
      const fileContent = zip.readFile(entry);
      if (fileContent !== null) {
        import_fs_extra5.default.outputFileSync(destPath, fileContent);
      }
    }
  });
  spinner.succeed(questionsText.downloadTemplate.spinner.succeed);
...
(async () => {
  ...
  if (templateType === "middleware") {
    ...
    const template = templatesList.middleware.find((m2) => m2.templateName === middleware);
